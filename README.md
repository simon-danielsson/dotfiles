...
### Native `lua/native/`
- 🔱 **trident.lua**: A Neovim-native interpretation of [harpoon](https://github.com/ThePrimeagen/harpoon/tree/harpoon2). Buffers are not added manually like in harpoon, but are instead added and subtracted from the list automatically. A maximum of 6 buffers are allowed to exist on the list at once to limit visual noise. The list automatically rotates when a listed buffer is closed.
